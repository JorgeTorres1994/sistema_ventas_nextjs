  ...
  return (
    <div className="bg-white rounded-xl p-6 border border-[#E5E7EB] shadow-sm h-[420px] flex flex-col">
      <div>
        ...
        <p className="text-[15px] text-[#6B7280]">Por canal de venta</p>
      </div>
      ...
      <div className="flex-1 relative mt-4">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          <span className="text-[11px] font-bold text-[#6B7280] tracking-wider uppercase">TOTAL</span>
        </div>
      </div>
          ...
          return (
            <div key={idx} className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[15px] text-[#4B5563] truncate max-w-[120px]">{
                  item.name === 'POS Terminal' ? 'Terminal POS' :
                  item.name === 'Online Shop' ? 'Tienda Online' :
                  item.name === 'Mobile App' ? 'App Móvil' : item.name
                }</span>
              </div>
